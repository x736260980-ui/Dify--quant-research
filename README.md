...
2. **并行数据采集**（条件分支）：
   ...
   - **路径 B**：Agent 接管 → **抖音搜索** → 精选 3 个视频 → 调用 **B站并发探针**（`/api/bili/batch-full`）批量获取弹幕+评论 + 调用 **抖音评论** 获取评论 → 情绪分析
3. **交叉验证分析** — DeepSeek-reasoner 融合四个维度数据（宏观资讯 + 量价因子 + B站舆情 + 抖音舆情），输出结构化报告
